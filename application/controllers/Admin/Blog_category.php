...
class Blog_category extends CI_Controller {
    ...
    public function index($id = '') {
        if (!empty($id)) {
            $data['edit_category'] = $this->db->get_where('tbl_blog_category', ['id' => $id])->row();
        }
        $data['blog_category'] = $this->Adminuser->select_Record('tbl_blog_category');
        $this->load->view('dashboard/header');
        $this->load->view('dashboard/blog_category', $data);
    ...
    public function add() {
        ...
        if (empty($edit_id)) {
            $categoryname = $this->db->get_where('tbl_blog_category', ['id >' => 0])->result();
        ...
        if (empty($edit_id)) {
            $slugname = $this->db->get_where('tbl_blog_category', ['id >' => 0])->result();
        ...
        // print_r($data);die();
        if (!empty($edit_id)) {
            $this->db->update('tbl_blog_category', $data, ['id' => $edit_id]);
            $result['status'] = 1;
